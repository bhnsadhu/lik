...
The artwork ships with the app itself at `/demo/*.webp` (committed under
`public/demo/`, generated by `scripts/gen-demo-photos.py`), **not** from
Supabase storage. Storage objects are owned by the auth user that
uploaded them, so deleting demo accounts orphans every URL their
profiles point at — that has already happened once and left the demo
feed with dead images.

The paths stored in the database are root-relative (`/demo/...`) rather
than absolute. `npx cap sync` copies `build/` into the iOS bundle, so
inside the app they resolve against `capacitor://localhost` and load off
disk — no network round trip, and nothing the CDN or firewall can
interrupt. Run `npx cap sync` before archiving so the bundled assets
match the seeded paths.
