...
export default function StepIndicator({ currentStep }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', padding: '0 18px 0', marginBottom: '14px' }}>
      {steps.map((step, i) => {
        const num = i + 1;
          ...
          <React.Fragment key={step}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px' }}>
              ...
              <span style={{
                ...
                fontSize: '9px',
                fontWeight: 300,
                color: active ? 'rgba(255,255,255,0.5)' : done ? 'rgba(61,220,255,0.6)' : 'rgba(255,255,255,0.2)'
              }}>{step}</span>
            </div>
            {i < steps.length - 1 && (
              <div style={{
